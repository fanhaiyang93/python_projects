...
# @Time    : 2018/3/8 18:35
# @Author  : fanhaiyang
# @File    : spider_jiandanMM.py
# @Software: PyCharm Community Edition
# @comment : 在煎蛋反爬情况下爬取妹子图，在Github上找的
import hashlib
import base64
...
class UrlMannager:
    def __init__(self):
        self.url_used = []
        self.url_target = queue.Queue()  # queue是一个同步队列的模块
        if os.path.exists('url.txt'):
            with open('url.txt', 'r') as f: # 文件读写的标准格式，就照这么写
    ...
    def getNewUrl(self):
        return self.url_target.get() # 从url队列中弹出一个url

    def isEmpty(self):
        return self.url_target.empty() # 判断url队列是否为空
...
if __name__ == '__main__':
    ...
    firstIndex = int(re.search(r'.*current.*\">\[(.*?)\].*', firstPage).group(1))  # 首页页码
